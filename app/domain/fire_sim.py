...
def apply_fire_tick(
    *,
    ...
    # Естественное затухание: каждый тик тепло снижается на 1
    for i in range(len(next_heat)):
        if next_heat[i] > 0:
            next_heat[i] = max(0, next_heat[i] - 1)

    # Генерация тепла: горящие клетки передают тепло соседям
    for index, cell_state in enumerate(state):
        if cell_state <= 0:
            continue

        # Тепловыделение зависит от материала
        ...
        # Передача тепла соседям с учётом расстояния и теплопроводности
    ...
    # Охлаждение стволами: -15 тепла в зоне поражения
    ...
    # Обновление состояния: воспламенение и тушение
    for index, cell_state in enumerate(state):
        if cell_state == 2:
