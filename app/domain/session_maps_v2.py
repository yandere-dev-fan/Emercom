...
def serialize_map_for_role(
    ...
    if map_document.kind == "object" and role in {"dispatcher", "observer"} and not (scenario_state and scenario_state.incident_revealed):
        hidden_layers = {layer.layer_key for level in map_document.levels for layer in level.layers}
    if map_document.kind == "object" and role == "rtp" and scenario_state is not None and not scenario_state.incident_revealed:
        hidden_layers = {layer.layer_key for level in map_document.levels for layer in level.layers}
