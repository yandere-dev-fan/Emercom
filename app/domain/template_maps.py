...
from app.domain.tile_catalog_v2 import DEFAULT_AREA_LEVELS, DEFAULT_LAYER_ORDER, DEFAULT_OBJECT_LEVELS, max_code_for_layer
...
def _seed_map_layers(map_document: MapDocument) -> None:
    for sort_order, (level_code, level_title) in enumerate(_levels_for_kind(map_document.kind), start=1):
...
def create_template_map(
    ...
    *,
    ...
    if payload.kind == "object" and payload.parent_map_id:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Для шаблонной карты объекта не используется parent_map_id.")
...
def update_template_map_metadata(
    db: Session,
    *,
    map_document: MapDocument,
    ...
    for change in payload.changes:
        ...
        for write in change.writes:
            if write.index >= expected_count:
                raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Индекс клетки вне допустимого диапазона.")
            if write.value > layer.max_code:
                raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Значение клетки превышает допустимый максимум слоя.")
...
def clone_template_to_session_map(
    ...
    *,
    ...
    for level_payload in template_map.levels:
        level = MapLevel(map_document=runtime_map, code=level_payload.code, title=level_payload.title, sort_order=level_payload.sort_order)
        db.add(level)
